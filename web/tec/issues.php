...
<head>
    ...
    <?php
    // Cargar configuración, sesión, funciones de BD y envío de correo
    require("../../etc/config.php");
    require("../../etc/sessionTec.php");
    include("../../etc/db_functions.php");
    require("../../phpmailer/sendmail.php");

    // Título dinámico según si se está viendo una incidencia concreta
    echo "<title>";
    if (isset($_GET['ver'])) {
        $id = $_GET['id'];
...
<body>
    ...
    <main>
        <?php
        // Si se está visualizando una incidencia concreta
        if (isset($_GET['ver'])) {
            echo "<div class='viewFullIssue'>\n";
            // Consultar datos de la incidencia
            $consulta = mysqli_query($bbdd, "SELECT ic.id AS 'idIncidencia', descripcion, urgente, fechaApertura, fechaCierreEsp, fechaCierre, estado, solucion, desplazamiento, us.nombre AS 'nombre', us.correo AS 'correo', us.telefono AS 'telefono', em.nombre AS 'razonSocial' FROM incidencias ic LEFT JOIN usuarios us ON ic.usuario = us.id LEFT JOIN empresas em ON us.empresa = em.id WHERE ic.id = $id");
            ...
            // Si está cerrada, mostrar solución y datos de cierre
            ...
            if ($incidencia['nombre'] == NULL) {
                echo "<p>El usuario que abrió esta incidencia ya no existe.</p>";
            } else {
            ...
            echo "<br>\n";
            // Áreas asociadas a la incidencia
            echo "<h3>Áreas</h3>\n";
            ...
            if (mysqli_num_rows($consultaIntervenciones) == 0) {
                echo "<p>Esta incidencia no tiene ninguna intervención.</p>\n";
            } else {
        ?>
                ...
                // Si la incidencia está abierta, permitir crear intervención
                if ($incidencia['estado'] == 1) {
                    $salida = "";
                    ...
                    <br>
                    <!-- Formulario para crear intervención -->
                ...
                <?php
                    ...
                    // Si está cerrada, no se pueden crear intervenciones
                    echo "<h4>La incidencia ya está cerrada, por lo que no puedes crear nuevas intervenciones.</h4>\n";
                }
                echo "<br>\n";
                // Si está abierta, permitir cerrar incidencia
                if ($incidencia['estado'] == 1) {
                    ...
                    if (isset($_POST['cerrarIncidencia'])) {
                        // Recoger datos del formulario de cierre
                        ...
                        // Actualizar incidencia como cerrada
                        if (actualizar("UPDATE incidencias SET solucion = '$solucion', fechaCierre = '$fechaCierre', duracion = '$duracion', estado = 0 WHERE id = $id")) {
                            // Enviar correo si el usuario existe
                            if ($incidencia['nombre'] != NULL) {
                                enviarCorreo("noreply.resolvepluses@gmail.com", $incidencia['correo'], "Incidencia resuelta", "incidenciaResuelta", ["id" => $incidencia['idIncidencia'], "fechaApertura" => date("d/m/Y", strtotime($incidencia['fechaApertura']))]);
                            }
                            header("Location: " . $_SERVER['PHP_SELF'] . "?id=$id&ver=true");
                        }
                    }
                    // Calcular duración total de intervenciones
                ...
                // Listado de incidencias (todas o solo abiertas)
                if (isset($_GET['todas']) and $_GET['todas'] == "true") {
                    ...
                    $instruccion = "SELECT ic.id AS 'id', us.nombre AS 'usuario', urgente, fechaApertura FROM incidencias ic LEFT JOIN usuarios us ON us.id = ic.usuario ORDER BY urgente DESC, fechaApertura";
                } else {
                    $instruccion = "SELECT ic.id AS 'id', us.nombre AS 'usuario', urgente, fechaApertura FROM incidencias ic LEFT JOIN usuarios us ON us.id = ic.usuario WHERE estado = 1 ORDER BY urgente DESC, fechaApertura";
                ...
                while ($incidencias = mysqli_fetch_array($consulta)) {
                    ...
                    echo "<td>";
                    if ($incidencias['usuario'] == NULL) {
                        echo "-";
                    } else {
                        echo $incidencias['usuario'];
                    }
                    echo "</td>\n";
